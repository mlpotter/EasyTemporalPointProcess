...
class CumulHazardFunctionNetwork(nn.Module):
    ...
    def __init__(self, model_config):
        ...
        self.layer_dense_1 = nn.Linear(in_features=1, out_features=self.hidden_size,bias=True)

        # concat rnn states and inter-event time embedding
        self.layer_dense_2 = nn.Linear(in_features=self.hidden_size*2, out_features=self.hidden_size,bias=True)

        # mlp layers
        self.module_list = nn.ModuleList(
            [nn.Linear(in_features=self.hidden_size, out_features=self.hidden_size,bias=True) for _ in
    ...
    def forward(self, hidden_states, time_delta_seqs):
        ...
        # t = self.layer_dense_1(torch.log(time_delta_seqs.unsqueeze(dim=-1) + 1.0)) if self.use_ln else self.layer_dense_1(time_delta_seqs.unsqueeze(dim=-1))  # [batch_size, seq_len, hidden_size]
        ...
        integral_lambda = (self.layer_dense_3(out) - self.layer_dense_3(out_base)) + out_bias * time_delta_seqs.unsqueeze(dim=-1)#+ F.softplus(self.bias_param * time_delta_seqs.unsqueeze(dim=-1))
        # integral_lambda = self.layer_dense_3(out) #+ out_bias * time_delta_seqs.unsqueeze(dim=-1)#+ F.softplus(self.bias_param * time_delta_seqs.unsqueeze(dim=-1))
        ...
        if self.proper_marked_intensities:
            ...
            # print(derivative_integral_lambda)

        return integral_lambda, derivative_integral_lambda


class FullyNNTransformer(TorchBaseModel):
    ...
    def __init__(self, model_config):
        ...
        """
        super(FullyNNTransformer, self).__init__(model_config)
        ...
        self.layer_input_projection = nn.Linear(1, embed_dim,bias=True)
        # self.layer_censor_emb = nn.Parameter(torch.zeros(1, embed_dim))  # for censoring, if needed (how do I make this better init?)
    ...
    def forward(self, time_seqs, time_delta_seqs, type_seqs, attention_mask_seqs):
        ...
        transformer_input = input_embed + position_embed  # [batch_size, seq_len, hidden_size * num_heads]
        # [batch_size, seq_len, hidden_size]
        # states right after the event
        hidden_states = self.layer_transformer(transformer_input,mask=attention_mask_seqs.repeat_interleave(self.num_heads, dim=0))
    ...
    def loglike_loss(self, batch):
        ...
        """
        # [batch_size, seq_len]
        time_seqs, time_delta_seqs, type_seqs, batch_non_pad_mask, batch_attention_mask, batch_len_seqs = batch
